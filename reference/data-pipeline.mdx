...
<CardGroup cols={2}>
  ...
  <Card title="Training shards" icon="circle-play">
    Up to 180 shards available; default download is 80 (8B tokens)
  </Card>
...
# All available shards (up to 180)
...
Wraps `TokenStream` for multi-GPU training. Each call to `next_batch()` takes a contiguous chunk of `TRAIN_BATCH_TOKENS + world_size` tokens from the stream, then slices out one disjoint `local_tokens + 1` span per rank. The extra token enables `(x, y)` construction by shifting.
